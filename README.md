...
| [Cortex](https://github.com/nikolmedo/cortex) | An answer engine that builds its own interface: a Gemini model returns the answer plus a presentation spec (intent, layout, mood, palette, module kinds) that a fixed set of React components renders, streamed over SSE so the UI themes itself in about a second and the grounded answer fills in progressively. | `TypeScript` `React` `Gemini` `Genkit` `SSE` |
